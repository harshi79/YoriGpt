...
import { listAppearancesForPet, resolveAppearanceForPet, resolvePet } from "../catalog";
import { requestPetAppearance, requestPetSelection } from "../client";
...
/**
 * The `/pets` playground: pick a pet, pick a mood, pick a size, watch the renderer.
 *
 * State and size are always local demo controls. The *pet* selection is persisted
 * for a signed-in user through `PUT /api/settings/pet`; an anonymous visitor keeps
 * the choice in this tab only and never writes to the database. A save is optimistic
 * — the new pet is drawn immediately, confirmed by the server's answer, and rolled
 * back with a short note if it fails — so a tiny preference never shows a spinner.
 *
 * The controls are ordinary buttons (keyboard operable, `aria-pressed`). The mood
 * caption is plain text and **not** a live region, so a pet changing mood never
 * interrupts a screen reader and no meaning rides on animation alone.
 */
export function PetPlayground({
  ...
  const choosePet = async (id: string) => {
    ...
    // An appearance that the new pet does not have drops back to that pet's default.
  ...
  return (
    <div className="pets-playground">
      <div className="pets-stage">
        <PetRenderer pet={pet} appearance={appearance.id} state={state} size={size} />
